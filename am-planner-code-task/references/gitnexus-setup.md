...
|----------|------|---------|
| Understand architecture before planning task | MCP Bridge | `node $BRIDGE query/context/impact` |
| Check blast radius before delegating | MCP Bridge | `node $BRIDGE impact '{"symbol":"X"}'` |
| Claude Code navigating/debugging | Claude Code MCP | Auto via MCP (if setup) |
| Check if index is stale | CLI | `gitnexus status` |
| Re-index after large changes | CLI | `gitnexus analyze` |
